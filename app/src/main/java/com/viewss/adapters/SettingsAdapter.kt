...
class SettingsAdapter(var list: ArrayList<SettingsModels>, var context: Context) :
    RecyclerView.Adapter<SettingsAdapter.viewHolder>() {

    inner class viewHolder(var binding: ItemSettingsBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(model: SettingsModels, position: Int) {
            binding.apply {
                ...
                root.setOnClickListener {
                    when (position) {
                        0 -> {
                            ...
                            val dialogBinding = DialogGuidesBinding.inflate((context as Activity).layoutInflater)
